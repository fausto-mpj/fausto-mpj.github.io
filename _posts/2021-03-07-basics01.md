...
The OLS is *mechanically* simple. Suppose we have a \\( n \times p \\) matrix \\( \mathbf{X} \\), a \\( p \times 1 \\) vector \\( \mathbf{\beta} \\), and a \\( n \times 1 \\) vector \\( \mathbf{y} \\), where the \\( i \\)-eth column of \\( \mathbf{X} \\) is denoted \\( \mathbf{x}_{i}_{} \\).
